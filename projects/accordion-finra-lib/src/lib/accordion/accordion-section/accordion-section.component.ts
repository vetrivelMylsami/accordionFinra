...
export class AccordionSectionComponent implements OnInit {
  ...
  ngOnInit() {
    this.accordion.addSection(this, this.title); //each accordion section is added to the array of sections through parent component's addSection method
  }

  toggleOpen() {
    this.accordion.sections.forEach(accordionEl => {
      if (accordionEl.section.isOpen === true) accordionEl.section.onClose.emit(); //if accordion-section is about to get closed, 'onClose' event emitter informs its inner component
    });
